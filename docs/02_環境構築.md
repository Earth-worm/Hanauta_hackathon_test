...
## ステップ2：チームのリポジトリをクローンする

**チーム専用のリポジトリは運営が事前に用意しています。** 代表者が作る必要はありません。
全員が同じことをやればOKです。
...
配られた **チームのリポジトリ URL** を使います。

```bash
git clone https://github.com/<配られたURL>/team-xxxx-hanalog.git
```

```bash
cd team-xxxx-hanalog
```
...
> 🔑 clone 時にログインを求められたら、GitHub のユーザー名と
> **パスワードの代わりにアクセストークン（PAT）** を入力します。
>
> ⚠️ clone できない場合は、GitHub Organization への招待を承認したか確認してください。
